...
let productName = "JsonSchemaProvider"

let sln = rootDirectory </> "JsonSchemaProvider.sln"

let dotNetPackTarget = rootDirectory </> "src" </> "JsonSchemaProvider.Runtime"

let srcCodeGlob =
    !!(rootDirectory </> "src/**/*.fs") ++ (rootDirectory </> "src/**/*.fsx")
    -- (rootDirectory </> "src/**/obj/**/*.fs")

let testsCodeGlob =
    !!(rootDirectory </> "tests/**/*.fs") ++ (rootDirectory </> "tests/**/*.fsx")
...
let revertChangelog _ =
    if String.isNotNullOrEmpty Changelog.changelogBackupFilename then
        Changelog.changelogBackupFilename |> Shell.copyFile changelogPath
